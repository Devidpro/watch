...
const checkIsBeforeSelectedDefault = (groupName) => {
  ...
  document.querySelector(`.${groupName}`).childNodes.forEach((child) => {
    if (
      +child.getAttribute("data-id") === defaultValues[groupName] &&
      child.classList.contains("select_button_active")
    ) {
...
const checkIsSelectedBefore = (groupName, number) => {
  ...
  document.querySelector(`.${groupName}`).childNodes.forEach((child) => {
    if (
      +child.getAttribute("data-id") === number &&
      child.classList.contains("select_button_active")
    ) {
...
const changeElement = (el, name, num) => {
  ...
  document.getElementById(
    name + "_watch"
  ).src = `https://raw.githubusercontent.com/sku757/konfig/main/image//${name}/${name}${num}.png`;
  ...
  if (
    !isSelectedSomeAdditionalBeforeChange &&
    isSelectedSomeAdditionalAfterChange
  ) {
    changePrice(0);
  }
  if (
...
const getElements = (lastIndex, groupName, defaultIndex = null) => {
  for (let i = 1; i <= lastIndex; i++) {
    const className =
      i === defaultIndex
        ? `class="select_button select_button_active"`
        : `class="select_button"`;
    document
...
getElements(
  groupsCountItems["displays"],
  "displays",
  defaultValues["displays"]
);
getElements(
  groupsCountItems["metal_body"],
  "metal_body",
  defaultValues["metal_body"]
);
...
getElements(
  groupsCountItems["metal_straps"],
  "metal_straps",
  defaultValues["metal_straps"]
);
...
getElements(
  groupsCountItems["clock_points"],
  "clock_points",
  defaultValues["clock_points"]
);
...
option_buttons.forEach((button) => {
  button.addEventListener("click", (e) => {
    if (
      (metal_strap_selected &&
        e.target.classList.contains("rubber_strap_option")) ||
      (!metal_strap_selected &&
        e.target.classList.contains("metal_strap_option"))
    ) {
      option_buttons.forEach((oldButton) => {
        oldButton.classList.toggle("selected_option");
      });
      const metalBlocks = document.querySelectorAll(
        ".metal_strap_option_selected"
      );
      metalBlocks.forEach((block) => {
        block.classList.toggle("hide");
      });
      const rubberBlocks = document.querySelectorAll(
        ".rubber_strap_option_selected"
      );
...
download_button.addEventListener("click", () => {
  ...
  setTimeout(() => {
    ...
    target.style.backgroundImage =
      "url('https://raw.githubusercontent.com/sku757/konfig/main/image/bg/bg.png')";
...
bluetooth_option.addEventListener("click", (e) => {
  ...
  if (bluetooth_option.checked) {
    ...
    bluetoothKeys.forEach((keyName) => {
      ...
      getBluetoothElements(
        keyName,
        optionsForBluetooth[keyName],
        defaultValues[keyName]
      );
      document.getElementById(
        `${keyName}_watch`
      ).src = `https://raw.githubusercontent.com/sku757/konfig/main/image//${keyName}/${keyName}${defaultValues[keyName]}.png`;
    });
  } else {
    changePrice(0);
    bluetoothKeys.forEach((keyName) => {
